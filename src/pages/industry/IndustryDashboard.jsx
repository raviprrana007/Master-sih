      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2vw, 24px)' }}>
        ...
        <div style={{
          ...
          <div className="card" style={{ padding: '1.25rem' }}>
            <h3 style={{ margin: '0 0 1rem', fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-1)' }}>Applications Trend</h3>
            <ResponsiveContainer width="100%" height={chartHeight}>
              <LineChart data={APP_TREND}>
                ...
                <Tooltip contentStyle={{ background: 'var(--surface-el)', border: '1px solid var(--border)', borderRadius: 8 }} />
                <Line type="monotone" dataKey="apps" stroke="#06B6D4" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          <div className="card" style={{ padding: '1.25rem' }}>
            <h3 style={{ margin: '0 0 1rem', fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-1)' }}>Top Candidate Skills</h3>
            <ResponsiveContainer width="100%" height={chartHeight}>
              <BarChart data={SKILL_DIST} layout="vertical">
                <XAxis type="number" tick={{ fill: 'var(--text-3)', fontSize: 11 }} axisLine={false} tickLine={false} />
                <YAxis dataKey="skill" type="category" tick={{ fill: 'var(--text-3)', fontSize: 11 }} axisLine={false} tickLine={false} width={60} />
                <Tooltip contentStyle={{ background: 'var(--surface-el)', border: '1px solid var(--border)', borderRadius: 8 }} />
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.875rem' }}>
            <h2 style={{ margin: 0, fontSize: '1rem', fontWeight: 600, color: 'var(--text-1)' }}>Top Matching Candidates</h2>
              ...
              <div key={c.id} className="card" style={{ padding: '1.25rem' }}>
                <div style={{ display: 'flex', gap: 10, marginBottom: '0.875rem' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-3)' }}>{c.institution} • CGPA {c.cgpa}</div>
                ...
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: '0.875rem' }}>
                  {c.skills.slice(0, 3).map(s => <span key={s} style={{ padding: '1px 6px', borderRadius: 4, fontSize: '0.7rem', background: 'rgba(6,182,212,0.08)', color: 'var(--text-2)', border: '1px solid rgba(6,182,212,0.15)' }}>{s}</span>)}
                </div>
        ...
        {/* Active job postings */}
        <div>
          <h2 style={{ margin: '0 0 0.875rem', fontSize: '1rem', fontWeight: 600, color: 'var(--text-1)' }}>Active Job Postings</h2>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['Job Title', 'Applications', 'Shortlisted', 'Status'].map(h => (
                    <th key={h} style={{ padding: '0.75rem 1rem', textAlign: 'left', fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-3)', textTransform: 'uppercase' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {ACTIVE_JOBS.map((j, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem', color: 'var(--text-1)', fontWeight: 500 }}>{j.title}</td>
                    <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem', color: 'var(--text-2)' }}>{j.apps}</td>
                    <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem', color: 'var(--text-2)' }}>{j.shortlisted}</td>
