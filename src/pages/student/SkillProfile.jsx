    ...
    <div style={{ maxWidth: 1000, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
        ...
        <div className="card" style={{ padding: '1.25rem' }}>
          <h3 style={{ margin: '0 0 1rem', fontSize: '0.9375rem', fontWeight: 600, color: '#e2e8f0' }}>Skill Radar</h3>
        ...
        <div className="card" style={{ padding: '1.25rem' }}>
          <h3 style={{ margin: '0 0 1rem', fontSize: '0.9375rem', fontWeight: 600, color: '#e2e8f0' }}>Career Compatibility</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {CAREER_COMPAT.map(c => (
              <div key={c.role}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ fontSize: '0.8rem', color: '#e2e8f0' }}>{c.role}</span>
      ...
      <div className="card" style={{ padding: '1.25rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h3 style={{ margin: 0, fontSize: '0.9375rem', fontWeight: 600, color: '#e2e8f0' }}>Technical Skills</h3>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          {[...TECH_SKILLS, ...customSkills.map((s, i) => ({ name: s, level: 50, verified: false, color: '#64748b' }))].map(s => (
      ...
      {/* Soft Skills */}
      <div className="card" style={{ padding: '1.25rem' }}>
        <h3 style={{ margin: '0 0 1rem', fontSize: '0.9375rem', fontWeight: 600, color: '#e2e8f0' }}>Soft Skills</h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
          {SOFT_SKILLS.map(s => (
            <div key={s.name} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontSize: '0.8rem', color: '#e2e8f0' }}>{s.name}</span>
                <span style={{ fontSize: '0.75rem', color: '#94a3b8' }}>{s.level}%</span>
              </div>
              <div style={{ height: 6, borderRadius: 3, background: 'rgba(255,255,255,0.06)' }}>
      ...
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="Add Skill">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <label style={{ display: 'block', fontSize: '0.75rem', color: '#94a3b8', marginBottom: 6, fontWeight: 600 }}>
          ...
          <div>
            <div style={{ fontSize: '0.75rem', color: '#64748b', marginBottom: 8 }}>Suggested skills</div>
